...
namespace LithosNet.Compiler {
    public class AstBuilder : LPCBaseVisitor<AstNode> {
        ...
        // 【二進位運算通用處理】
        private AstNode HandleBinaryOps(ParserRuleContext context, string[] childMethodNames, string[] ops) {
            // 簡化版：直接遍歷子節點
            AstNode node = Visit(context.GetChild(0));
        ...
        public override AstNode VisitLogicalOrExpr(LPCParser.LogicalOrExprContext c) => HandleBinaryOps(c, null, null);
        public override AstNode VisitLogicalAndExpr(LPCParser.LogicalAndExprContext c) => HandleBinaryOps(c, null, null);
        public override AstNode VisitEqualityExpr(LPCParser.EqualityExprContext c) => HandleBinaryOps(c, null, null);
        public override AstNode VisitRelationalExpr(LPCParser.RelationalExprContext c) => HandleBinaryOps(c, null, null);
        public override AstNode VisitAdditiveExpr(LPCParser.AdditiveExprContext c) => HandleBinaryOps(c, null, null);
        public override AstNode VisitMultiplicativeExpr(LPCParser.MultiplicativeExprContext c) => HandleBinaryOps(c, null, null);
...
with open("LithosNet.Compiler/AstBuilder.cs", "w", encoding="utf-8") as f:
    f.write(code)
print("✅ AstBuilder.cs (基於反射的終極橋接器) 已建立！")
